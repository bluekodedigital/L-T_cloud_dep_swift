    ...
    function files_inom($pid, $segment)
    {
        ...
        if ($pid == "") {
            $sql = "select  distinct name,om_packid,om_projid,om_recvuid,om_recv_stageid,proj_name,om_remarks,om_sentdate,
                    ...
                    and stage_id=om_recv_stageid  and uid=om_senderuid and om_active=1 and om_status=0 $cond";
        } else {
            $sql = "select  distinct name,om_packid,om_projid,om_recvuid,om_recv_stageid,proj_name,om_remarks,om_sentdate,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date as planned,ps_expdate, pm_createdate as expected,GETDATE() as actual,stage_name 
                    from usermst,swift_OandM,swift_packagemaster,swift_packagestatus,Project,swift_stage_master where proj_id=om_projid and
                    om_packid=pm_packid and om_packid=ps_packid and om_projid='" . $pid . "' and ps_stageid=om_recv_stageid  
                    and stage_id=om_recv_stageid   and proj_id='" . $pid . "' and uid=om_senderuid and om_active=1 and om_status=0 $cond";
        }
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
...
and stage_id=tech_recv_stageid  and ps_packid='" . $pack_id . "' and uid=tech_senderuid and tech_active=1 and swift_techapproval.tech_status='" . $flag . "'";
    ...
    function files_for_poentry($pid, $segment)
    {
        $uid = $_SESSION['uid'];
        if ($pid == "") {
            //     $sql = "SELECT count(cs_packid) as rwcount from swift_workflow_CurrentStage as a
            //     left join  Project as b on a.cs_projid=b.proj_id where to_uid =  '$uid' and  cs_active= 0 and to_stage_id=16
            //    and cat_id='" . $segment . "' ";
            $sql = "SELECT count(pw_packid) as rwcount from Swift_po_wo_Details  as a
        left join  Project as b on a.pw_projid=b.proj_id ";
        } else {
            // $sql = "SELECT count(cs_packid) as rwcount from swift_workflow_CurrentStage as a
            // left join  Project as b on a.cs_projid=b.proj_id
            // where   to_uid =  '$uid' and  cs_active= 0 and to_stage_id=16
            // and cat_id='" . $segment . "'  and   cs_projid='" . $pid . "'";
            $sql = "SELECT count(pw_packid) as rwcount from Swift_po_wo_Details  as a
        left join  Project as b on a.pw_projid=b.proj_id where  pw_projid='" . $pid . "'";
        }
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        $count = $row['rwcount'];
    ...
    {
        // $sql = "select * from swift_po_master where po_pack_id='" . $pack_id . "'";
        $sql = "select * from dbo.swift_po_master AS A left outer join swift_poentrysave AS B 
      on A.po_id=B.sweid where po_pack_id='" . $pack_id . "'";
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
            $result[] = $row;
    ...
    {
        $sql = "select proj_type,pm_packagename,proj_name from swift_packagemaster,Project where pm_projid=proj_id and pm_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
    ...
    {
        if ($pid == "") {
            $sql = "select pm_createdate,ps_userid,proj_id,ps_packid,pm_packagename,proj_name,stage_name,ps_stageid,isnull(DATEDIFF(DAY,revised_planned_date,ps_actualdate),0) as daysdif,ps_sbuid,ps_stback,
 ...
 and ps_stageid=stage_id and cat_id='" . $segment . "'";
            ...
            $sql = "select pm_createdate,ps_userid,proj_id,ps_packid,pm_packagename,proj_name,stage_name,ps_stageid,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as daysdif,ps_sbuid,ps_stback,
 revised_planned_date as planned,ps_expdate, ps_actualdate as acutal,pm_material_req,pm_revised_material_req,pm_revised_lead_time
 from swift_packagestatus,Project,swift_packagemaster,swift_stage_master 
 where proj_id=ps_projid and ps_packid=pm_packid and  active=1
 and ps_stageid=stage_id and cat_id='" . $segment . "' and proj_id='" . $pid . "'";
    ...
    function repo_firstrow($pack_id, $projid, $flg)
    {
        if ($flg == 1) {
            //            echo $sql = " select top 10 a.*,b.altShortName,b.stage_id,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
//  where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "'  and a.ps_projid ='" . $projid . "'
//  order by ps_stageid ASC ";
            $sql = "select top 10 c.*,d.altShortName, d.stage_id, d.stage_name, shot_name,
            DATEDIFF(DAY, revised_planned_date, ps_actualdate) AS days  from swift_packagemaster as a 
            inner join swift_workflow_details as b on a.pm_wfid=b.mst_id
            inner join swift_packagestatus as c on a.pm_packid=c.ps_packid and b.stage_id=c.ps_stageid
            left join swift_stage_master as d on d.stage_id=b.stage_id and d.stage_id=c.ps_stageid
            where ps_packid='$pack_id' and ps_projid='$projid' order by b.Did asc";
        } else {
            $sql = "select top 10 c.*,d.altShortName, d.stage_id, d.stage_name, shot_name,
            DATEDIFF(DAY, revised_planned_date, ps_actualdate) AS days  from swift_packagemaster as a 
            inner join swift_workflow_details as b on a.pm_wfid=b.mst_id
    ...
    function repo_firstrow_version1($pack_id, $flg)
    {

        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        if ($flg == 1) {
            $sql = " select top 10 a.*,b.altShortName,b.stage_id,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' 
 order by ps_stageid ASC ";
        } else {
            $sql = " select top 10 a.*,b.altShortName,b.stage_id,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "'
 order by ps_stageid ASC  ";
    ...
    function repo_secondrow($pack_id, $projid, $poc)
    {
        if ($poc == 0) {
...
//  where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_projid ='" . $projid . "'
//  and a.ps_stageid >=11  order by ps_stageid ASC ";
            $sql = "select  c.*,d.altShortName, d.stage_id, d.stage_name, shot_name,
DATEDIFF(DAY, revised_planned_date, ps_actualdate) AS days  from swift_packagemaster as a 
inner join swift_workflow_details as b on a.pm_wfid=b.mst_id
inner join swift_packagestatus as c on a.pm_packid=c.ps_packid and b.stage_id=c.ps_stageid
left join swift_stage_master as d on d.stage_id=b.stage_id and d.stage_id=c.ps_stageid
where ps_packid='$pack_id' and ps_projid='$projid'  order by b.Did asc offset 10 rows fetch next 10 rows only";
        } else {
            $sql = "select  c.*,d.altShortName, d.stage_id, d.stage_name, shot_name,
            DATEDIFF(DAY, revised_planned_date, ps_actualdate) AS days  from swift_packagemaster as a 
            inner join swift_workflow_details as b on a.pm_wfid=b.mst_id
    ...
    function repo_secondrow_version1($pack_id, $poc)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        if ($poc == 0) {
 ...
 and a.ps_stageid >=11 order by ps_stageid ASC ";
    ...
    function repo_secondrow_wo($pack_id, $poc)
    {
        if ($poc == 0) {
            $sql = "select a.*,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('4','7','10','11','13','12','15','24','25','20','21','25')
 and a.ps_stageid >=14 and a.ps_stageid<=25 order by ps_stageid ASC ";
        } else {
            $sql = "select a.*,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('4','7','10','11','13','12','15','24','20','21','25')
 and a.ps_stageid >=14 and a.ps_stageid<=25 order by ps_stageid ASC ";
        }
    ...
    function repo_secondrow_po($pack_id, $poc)
    {
        if ($poc == 0) {
            $sql = "select a.*,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('4','7','10','11','13','12','15','24','25','22','23','25')
 and a.ps_stageid >=14 and a.ps_stageid<=25 order by ps_stageid ASC ";
        } else {
            $sql = "select a.*,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days from swift_packagestatus as a,swift_stage_master as b 
 where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('4','7','10','11','13','12','15','24','22','23','25')
 and a.ps_stageid >=14 and a.ps_stageid<=25 order by ps_stageid ASC ";
        }
 ...
 and a.ps_stageid >25 and a.ps_stageid<=34 order by ps_stageid ASC ";
    ...
    function package_details($pack_id, $projid)
    {
        $sql = "select a.*,b.stage_name,shot_name,c.pm_revised_material_req,c.pm_revised_lead_time,c.pm_packagename,d.proj_name,
           DATEDIFF(DAY,a.revised_planned_date,a.ps_actualdate) as daysdif,ps_expdate,pm_createdate,d.proj_type
from swift_packagestatus as a,swift_stage_master as b, swift_packagemaster as c,Project as d
where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and ps_projid='" . $projid . "' and a.active=1 and a.ps_packid=c.pm_packid and d.proj_id=a.ps_projid
order by ps_stageid ASC ";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    ...
    function package_details_version1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
...
order by ps_stageid ASC ";
    ...
    function vendor_details_version1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
    ...
    function select_buyer($milcom = '')
    {
        ...
        $sql = "SELECT * FROM usermst WHERE usertype= '2' $cond";
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
            $result[] = $row;
    ...
    function om_count($pid, $seg)
    {
        ...
        if ($pid == "") {
            $sql = "select COUNT(*) as total from swift_OandM,Project where om_active=1 and om_status=0 and proj_id=om_projid $cond";
        } else {
            $sql = "select COUNT(*) as total from swift_OandM,Project where om_active=1 and om_status=0 and proj_id=om_projid  and proj_id='" . $pid . "' $cond";
        }
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        $count = $row['total'];
...
(select pm_packid from dbo.swift_packagemaster,Project where pm_projid=proj_id $segment and pm_projid='" . $pid . "' )";
    ...
    function om_repo($pid, $seg)
    {
        ...
        if ($pid == "") {
            $sql = "select COUNT(*) as total from swift_OandM,Project where om_active=1 and om_status=1 and proj_id=om_projid $cond";
        } else {
            $sql = "select COUNT(*) as total from swift_OandM,Project where om_active=1 and om_status=1 and proj_id=om_projid  and proj_id='" . $pid . "' $cond";
        }
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        $count = $row['total'];
    ...
    function files_inopsrepo($pid, $seg)
    {
        if ($seg != '') {
            $cond = " AND cat_id='" . $seg . "'";
        } else {
        ...
        if ($pid == "") {
            $sql = "select   name,om_packid,om_projid,om_recvuid,om_recv_stageid,proj_name,om_remarks,om_sentdate,
                    ...
                    from usermst,swift_OandM,swift_packagemaster,swift_packagestatus,Project,swift_stage_master where proj_id=om_projid and
                    om_packid=pm_packid and om_packid=ps_packid  and ps_stageid=om_recv_stageid  
                    and stage_id=om_recv_stageid  and uid=om_senderuid and om_active=1 and om_status=1 $cond";
        } else {
            $sql = "select   name,om_packid,om_projid,om_recvuid,om_recv_stageid,proj_name,om_remarks,om_sentdate,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date as planned, pm_createdate as expected,GETDATE() as actual,ps_expdate,ps_actualdate,stage_name 
                    from usermst,swift_OandM,swift_packagemaster,swift_packagestatus,Project,swift_stage_master where proj_id=om_projid and
                    om_packid=pm_packid and om_packid=ps_packid and om_projid='" . $pid . "' and ps_stageid=om_recv_stageid  
                    and stage_id=om_recv_stageid   and proj_id='" . $pid . "' and uid=om_senderuid and om_active=1 and om_status=1 $cond";
        }
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
    ...
    function files_from_smartsignloi($pid, $uid, $utype, $seg)
    {
        if ($seg != '') {
        ...
        if ($pid == "") {
            $sql = "select distinct bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,max(so_package_approved_date) as so_package_approved_date,
                    ...
                    and stage_id=18  and so_status=0 and bu_packid=so_pack_id  and bu_status=1 " . $cond . " " . $cond1 . "
                    group  by bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date, pm_createdate,stage_name,hpc_app";
        } else {
            $sql = "select distinct bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,max(so_package_approved_date) as so_package_approved_date,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date as planned, pm_createdate as expected,GETDATE() as actual,stage_name,hpc_app  
                    from swift_filesfrom_smartsignoff,swift_packagemaster,swift_packagestatus,Project,swift_stage_master,swift_SCMtoBUYER  
                    where proj_id=so_proj_id and so_pack_id=pm_packid and so_pack_id=ps_packid  and ps_stageid=18 
                    and stage_id=18  and bu_packid=so_pack_id " . $cond . " " . $cond1 . " and proj_id='" . $pid . "' and so_status=0 and bu_status=1
                    group  by bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date, pm_createdate,stage_name,hpc_app ";
        }
...
where pm_projid=proj_id and ps_stageid=stage_id and ps_packid=pm_packid and emr_status=0  and stage_id=19 and cat_id='" . $segment . "' and proj_id='" . $pid . "'";
    ...
    function files_for_emrcreation_afterloi($pid, $uid, $seg)
    {
        if ($seg != '') {
            $cond1 = " AND cat_id=$seg";
        } else {
        ...
        if ($pid == "") {
            $sql = "SELECT proj_type,pm_packid,pm_projid,proj_name,pm_packagename,stage_name,revised_planned_date as planned,GETDATE() as actual,
...
inner join swift_stage_master as g on g.stage_id=14 
    ...
    function filesforemr_ajax($pack_id)
    {
        $sql = "SELECT pm_packid,pm_projid,proj_name,pm_packagename,stage_name,revised_planned_date as planned,GETDATE() as actual,ps_expdate,pm_material_req,pm_revised_material_req
        from  swift_packagemaster,Project,swift_packagestatus,swift_stage_master
        where pm_projid=proj_id and ps_stageid=stage_id and ps_packid=pm_packid and emr_status='3'  and stage_id=14 and pm_packid='" . $pack_id . "' ";
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
            $result[] = $row;
...
where ps_stageid=stage_id and ps_stageid=16 and ps_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
    ...
    {
        $sql = "select ps_packid,ps_stageid,revised_planned_date as planned,ps_expdate as expected from swift_packagestatus,swift_stage_master 
where ps_stageid=stage_id and ps_stageid=17 and ps_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
    ...
    {
        $sql = "select ps_packid,ps_stageid,revised_planned_date as planned,ps_expdate as expected from swift_packagestatus,swift_stage_master 
where ps_stageid=stage_id and ps_stageid=18 and ps_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
...
where ps_stageid=stage_id and ps_stageid=18 and ps_packid='" . $pack_id . "'";
    ...
    {
        $sql = "select ps_packid,ps_stageid,revised_planned_date as planned,ps_expdate as expected from swift_packagestatus,swift_stage_master 
where ps_stageid=stage_id and ps_stageid=19 and ps_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
    ...
    {
        $sql = "select ps_packid,ps_stageid,revised_planned_date as planned,ps_expdate as expected from swift_packagestatus,swift_stage_master 
where ps_stageid=stage_id and ps_stageid=20 and ps_packid='" . $pack_id . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }
    ...
    function files_for_podetailsalter($pid, $segment)
    {
        if ($pid == "") {
            ...
            left join swift_poentrysave as b on a.po_id=b.sweid
            left join swift_packagemaster as c on c.pm_packid=a.po_pack_id
            left join Project as d on d.proj_id=a.po_projid 
           
          group by proj_name, pm_packagename,po_number,pm_revised_material_req,po_pack_id,proj_type";
        } else {
            $sql = "select distinct proj_type,proj_name,po_pack_id,pm_packagename,po_number,pm_revised_material_req,count(*) as count ,SUM(po_scope_qty) as total_Qty,((SUM(mqty)/SUM(po_scope_qty))*100) as manuQty,((SUM(iqty)/SUM(po_scope_qty))*100) as insQty,
            ((SUM(mdccqty)/SUM(po_scope_qty))*100) as mdccQty,((SUM(cclr_qty)/SUM(po_scope_qty))*100) as cclrQty,
            ((SUM(mrqty)/SUM(po_scope_qty))*100) as mrecQty
...
group by proj_name, pm_packagename,po_number,pm_revised_material_req,po_pack_id,proj_type";
    ...
    function check_poc_version1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select isnull(cm_poc_required,1) as cm_poc_required from swift_checklist_mapping where cm_pack_id='" . $pack_id . "' ";
...
where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid =33 ";
    ...
    function get_stage1plan($pack_id, $projid, $stageid)
    {
        if ($stageid == 2) {
            $sql = " select top 1* from  swift_packagestatus 
 where ps_packid='$pack_id'  and ps_planneddate !='' and ps_stageid <= '" . $stageid . "' and 
'$stageid' <= (select ps_stageid from swift_packagestatus where active=1 and ps_packid='$pack_id' and ps_projid=$projid)  
and ps_projid=$projid  order by ps_stageid DESC";
    ...
    function get_stage1actual($pack_id, $projid, $stageid)
    {
        if ($stageid == 2) {
            $sql = " select pm_createdate as ps_actualdate from swift_packagemaster where pm_packid ='$pack_id'";
    ...
    function get_actualreceive($pack_id, $projid, $stageid = '')
    {
        if ($stageid == 2) {
            $sql = " select top 1* from  swift_packagestatus 
...
and ps_projid=$projid order by ps_stageid DESC";
    ...
    function get_actualreceive_version1($pack_id, $stageid)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        if ($stageid == 2) {
...
'$stageid' <= (select ps_stageid from swift_packagestatus where active=1 and ps_packid='$pack_id')  
 order by ps_stageid DESC";
    ...
    function get_actualtarget($pack_id, $projid, $stageid)
    {
        if ($stageid == 2) {
            $sql = " select top 1* from  swift_packagestatus 
 where ps_packid='$pack_id'  and ps_actualdate !='' and ps_stageid <= '" . $stageid . "' and 
...
and ps_projid=$projid order by ps_stageid DESC";
    ...
    function get_planreceive($pack_id, $projid, $stageid)
    {
        if ($stageid == 2) {
            $sql = " select top 1* from  swift_packagestatus 
 where ps_packid='$pack_id'  and ps_actualdate !='' and ps_stageid <= '" . $stageid . "' and 
...
and ps_projid=$projid order by ps_stageid DESC";
    ...
    function files_from_smartsignloi_filter($pid, $uid, $hpc, $utype, $seg)
    {
        if ($seg != '') {
            $cond1 = " AND cat_id='" . $seg . "'";
        } else {
        ...
        if ($pid == "") {
            $sql = "select distinct bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,so_package_approved_date,
                    ...
                    from swift_filesfrom_smartsignoff,swift_packagemaster,swift_packagestatus,Project,swift_stage_master,swift_SCMtoBUYER  
                    where proj_id=so_proj_id and so_pack_id=pm_packid and so_pack_id=ps_packid  and ps_stageid=18 
                    and stage_id=18  and so_status=0 and bu_packid=so_pack_id " . $cond . " " . $cond1 . " and hpc_app ='" . $hpc . "'";
        } else {
            $sql = "select distinct bu_buyer_id,so_pack_id,so_proj_id,proj_name,ps_expdate,so_package_approved_date,
                    pm_packagename,pm_material_req,pm_revised_material_req,revised_planned_date as planned, pm_createdate as expected,GETDATE() as actual,stage_name,hpc_app  
                    from swift_filesfrom_smartsignoff,swift_packagemaster,swift_packagestatus,Project,swift_stage_master,swift_SCMtoBUYER  
                    where proj_id=so_proj_id and so_pack_id=pm_packid and so_pack_id=ps_packid  and ps_stageid=18 
                    and stage_id=18  and bu_packid=so_pack_id " . $cond . " " . $cond1 . " and proj_id='" . $pid . "' and so_status=0 and hpc_app ='" . $hpc . "'";
        }
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
    ...
    function pre_fetch_stage($pack_id, $stagid)
    {

        $sql = "select distinct shot_name from swift_stage_master as a
        left join swift_repository as r on r.rs_to_stage=a.stage_id and rs_active=1
        where rs_packid='$pack_id'  ";
        $query = mssql_query($sql);
        $row = mssql_fetch_assoc($query);
        $stagename = $row['shot_name'];
        return $stagename;
    }
    ...
    {
        $sql = "select * from  [dbo].[swift_po_wo_details] where pw_packid='" . $pack_id . "'";
    ...
    function get_po_wo_app_version1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select * from  [dbo].[swift_filesfrom_smartsignoff] where so_pack_id='" . $pack_id . "'";
...
where  a.active=1  and a.ps_packid='" . $packid . "'";
    ...
    function current_status2($packid, $flg)
    {
        $sql = "select c.altShortName,c.stage_id,d.name as sname,b.name,b.usertype,c.shot_name,(isnull(DATEDIFF(DAY,a.ps_actualdate,GETDATE()),0)) as nodays, a.* from  dbo.swift_packagestatus as a
left join usermst as b on a.ps_userid=b.uid
left join swift_stage_master as c on a.ps_stageid=c.stage_id  
left join usermst as d on a.ps_sbuid=d.uid
where  a.active=1  and a.ps_packid='" . $packid . "'";
        ...
        if ($sent_back == 1) {
            ...
            if ($row['name'] == "") {
                $status = $stagename . '<br>(' . $row['name'] . ' / ' . $row['nodays'] . ' Days )';
            } else {
                $status = $stagename . '<br>(' . $row['name'] . ' / ' . $row['nodays'] . ' Days )';
            }
        }
    ...
    function check_hpc_version1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select * from  [dbo].[swift_filesfrom_smartsignoff] where so_pack_id='" . $pack_id . "' and hpc_app=1 and so_status=1 ";
    ...
    function get_approver_name_v1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select distinct b.Sol_id,b.Quote_id,d.name,e.dep_name,f.hpc_app,f.so_status from solution as a,Quote_Approval as b,Quote_status as c,
...
d.deptid = e.dep_id    and a.swift_packid='" . $packid . "'   ";
    ...
    function files_for_poentry_projusers($pid, $segment)
    {

        $sql = "select COUNT(*) as techcount
                    from swift_techapproval,Project 
                    where proj_id=tech_projid and cat_id='" . $segment . "' and  tech_active=1 and swift_techapproval.tech_status =4 and proj_id in($pid)";
    ...
    function projusers_projects($uid)
    {
        $sql = "select * from  project_users where proj_uid='" . $uid . "'";
        $query = mssql_query($sql);
        $row = mssql_fetch_assoc($query);
        return $row;
    }

    function files_for_podetailsalter_projusers($pid, $segment)
    {

        $sql = "select distinct proj_type,proj_name,po_pack_id,pm_packagename,po_number,pm_revised_material_req,count(*) as count ,SUM(sqty) as total_Qty,((SUM(mqty)/SUM(sqty))*100) as manuQty,((SUM(iqty)/SUM(sqty))*100) as insQty,
...
where  cat_id='" . $segment . "' and proj_id in($pid)
group by proj_name, pm_packagename,po_number,pm_revised_material_req,po_pack_id";
    ...
    function repo_table1($pack_id, $poc, $flg, $projid)
    {

        if ($poc == 0) {
            if ($flg == 1) {
                $sql = "select d.pm_material_req,d.pm_createdate, a.st_projid,a.st_stageid,a.st_uid,c.ps_stback,a.st_remarks,a.st_status,b.stage_name,b.shot_name,c.ps_planneddate,c.ps_expdate,c.ps_actualdate,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days,c.revised_planned_date,f.target_day,rs_to_stage from 
                dbo.swift_transactions as a left join swift_stage_master as b on b.stage_id=a.st_stageid
                left join swift_packagestatus as c on ps_packid=st_packid and ps_projid=st_projid and st_stageid=ps_stageid
                left join swift_packagemaster as d on d.pm_packid=a.st_packid and d.pm_projid=a.st_projid 
               left join swift_workflow_details as f on f.stage_id=a.st_stageid and d.pm_wfid=mst_id
               left join swift_repository as r on r.rs_packid=a.st_packid and rs_active=1
                where st_packid='" . $pack_id . "' and st_projid='" . $projid . "' 
                order by st_id  ASC";
            } else {
            ...
            if ($flg == 1) {
                $sql = "select d.pm_material_req,d.pm_createdate, a.st_projid,a.st_stageid,a.st_uid,
                c.ps_stback,a.st_remarks,a.st_status,b.stage_name,b.shot_name,c.ps_planneddate,
                c.ps_expdate,c.ps_actualdate,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days,
                c.revised_planned_date,f.target_day from 
                dbo.swift_transactions as a left join swift_stage_master as b on b.stage_id=a.st_stageid
                left join swift_packagestatus as c on ps_packid=st_packid and ps_projid=st_projid and st_stageid=ps_stageid
                left join swift_packagemaster as d on d.pm_packid=a.st_packid and d.pm_projid=a.st_projid 
                left join swift_workflow_details as f on f.stage_id=a.st_stageid and d.pm_wfid=mst_id
    ...
    function repo_table1_version1($pack_id, $poc, $flg, $projid)
    {
        ...
        if ($poc == 0) {
            if ($flg == 1) {
                $sql = "select  a.st_projid,a.st_stageid,a.st_uid,a.st_remarks,b.stage_name,b.shot_name,c.ps_planneddate,c.ps_expdate,c.ps_actualdate,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days,c.revised_planned_date  from 
 dbo.swift_transactions as a left join swift_stage_master as b on b.stage_id=a.st_stageid
 right outer join swift_packagestatus as c on ps_packid=st_packid and ps_projid=st_projid and st_stageid=ps_stageid
 where st_packid='" . $pack_id . "' and st_projid='" . $projid . "' and ps_stageid not in('7','10','11','15','24') and st_stageid <24
 order by st_id  ASC";
 ...
 dbo.swift_transactions as a left join swift_stage_master as b on b.stage_id=a.st_stageid
 right outer join swift_packagestatus as c on ps_packid=st_packid and ps_projid=st_projid and st_stageid=ps_stageid
 where st_packid='" . $pack_id . "' and st_projid='" . $projid . "' and ps_stageid not in('4','7','10','11','15','24') and st_stageid <24
 order by st_id  ASC";
 ...
 order by st_id  ASC";
                ...
                $sql = "select  a.st_projid,a.st_stageid,a.st_uid,a.st_remarks,b.stage_name,b.shot_name,c.ps_planneddate,c.ps_expdate,c.ps_actualdate,c.revised_planned_date,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from 
 dbo.swift_transactions as a left join swift_stage_master as b on b.stage_id=a.st_stageid
 right outer join swift_packagestatus as c on ps_packid=st_packid and ps_projid=st_projid and st_stageid=ps_stageid
 where st_packid='" . $pack_id . "' and st_projid='" . $projid . "' and ps_stageid not in ('4','7','10','11','15','24') and st_stageid <24
 order by st_id  ASC";
    ...
    //    function repo_table1($pack_id, $poc, $flg) {
    //
    //
    //
    //        if ($poc == 0) {
    //            if ($flg == 1) {
    //                $sql = "select a.*,b.altShortName,b.stage_id,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
    // where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('7','10','11','15','24') and a.ps_stageid <24
    // order by ps_stageid ASC";
    //            } else {
    //                $sql = "select a.*,b.altShortName,b.stage_id,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
    // where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('4','7','10','11','15','24') and a.ps_stageid <24
    // order by ps_stageid ASC ";
    //            }
    //        } else {
    //            if ($flg == 1) {
    //                $sql = "select a.*,b.altShortName,b.stage_id,b.stage_name,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
    // where a.ps_stageid=b.stage_id and a.ps_packid='" . $pack_id . "' and a.ps_stageid not in('7','10','11','15','24') and a.ps_stageid <24
    // order by ps_stageid ASC ";
    //            } else {
    //                $sql = "select a.*,b.altShortName,b.stage_id,b.stage_name,b.stage_name,shot_name,DATEDIFF(DAY,revised_planned_date,ps_actualdate) as days  from swift_packagestatus as a,swift_stage_master as b 
    ...
    function getExpectedSSApproval($pack_id)
    {
        $sql = "select * from  swift_packagestatus where ps_packid='" . $pack_id . "' and ps_stageid=12";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }

    function getExpectedSSApproval_v1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select * from  swift_packagestatus where ps_packid='" . $pack_id . "' and ps_stageid=12";
    ...
    function getExpectedInitiateSSApproval($pack_id)
    {
        $sql = "select * from  swift_packagestatus where ps_packid='" . $pack_id . "' and ps_stageid=11";
        $query = mssql_query($sql);
        $row = mssql_fetch_array($query);
        return $row;
    }

    function getExpectedInitiateSSApproval_v1($pack_id)
    {
        $con = new dbcon2();
        $dbcon1 = $con->getValue();
        $sql = "select * from  swift_packagestatus where ps_packid='" . $pack_id . "' and ps_stageid=11";
    ...
    function fetchreviewer($seg, $milcom)
    {
        $sql = " select * from usermst where active='1' AND usertype=18 and milcom_app='" . $milcom . "'";
        //        $sql = " select * from usermst where active='1' AND usertype=18 AND  ','+tech_seg + ',' like '%,$seg,%'";
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
            $result[] = $row;
    ...
    function swift_techDocMst()
    {
        $sql = "select * from swift_techDocMst  where stActive=1";
        $query = mssql_query($sql);
        $result = array();
        while ($row = mssql_fetch_array($query)) {
            $result[] = $row;
    ...
    function fetchcto($seg, $milcom)
    {
        $sql = " select * from usermst where active='1' AND usertype=12 and milcom_app='" . $milcom . "'";
