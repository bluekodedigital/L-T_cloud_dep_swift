...
$pcrdate = formatDate($getExpectedInitiateSSApproval['ps_actualdate'], 'd-M-y');
...
/* if ($deviations < 0) {
    ...
    if (isset($getExpectedSSApproval['ps_actualdate']) && $getExpectedSSApproval['ps_actualdate'] instanceof DateTime) {
        $psActualDate = $getExpectedSSApproval['ps_actualdate']->format('Y-m-d');
    } else {
        $psActualDate = trim($getExpectedSSApproval['ps_actualdate'] ?? '');
    }
    ...
    if (isset($getExpectedInitiateSSApproval['ps_actualdate']) && $getExpectedInitiateSSApproval['ps_actualdate'] instanceof DateTime) {
        $psActualInitiateDate = $getExpectedInitiateSSApproval['ps_actualdate']->format('Y-m-d');
    } else {
        $psActualInitiateDate = trim($getExpectedInitiateSSApproval['ps_actualdate'] ?? '');
    }
                                        ...
                                        <li>
                                            <div class="timeline_content" style="font-size: 13px;margin-left:-61px; margin-right:-64px;">
                                                ...
                                                <?php if ($value['st_stageid'] == 2) { ?>
                                                    <p>Plan Receipt:
                                                        <?php  
                                                        $get_planrec = $cls_user->get_stage1plan($pack_id,$projid, $value['st_stageid']);
                                                        $res = json_decode($get_planrec);
                                                        $get_actrec = $cls_user->get_stage1actual($pack_id,$projid, $value['st_stageid']);
                                                        $res2 = json_decode($get_actrec);
                                                        if ($get_planrec == '0') {
                                                        } else {
                                                ...
                                                <?php } else {
                                                    ...
                                                    //For Plan
                                                    $get_planreceive = $cls_user->get_planreceive($pack_id, $projid,$value['st_stageid']);
                                                  ...
                                                  //  echo $target_day;
                                                     $plan_target  = date('d-M-y', strtotime(formatDate($rec2, 'Y-m-d'). ' + ' . (int)$target_day . ' days'));
                                                    $p_date = formatDate($plan_target, 'd-M-y');
                                                    $plantarget = formatDate($plan_target, 'd-M-y');
                                                    //For Actual
                                                    $get_actualreceive = $cls_user->get_actualreceive($pack_id, $projid,$value['st_stageid']);
                                                    ...
                                                    $get_actualtarget = $cls_user->get_actualtarget($pack_id,$projid, $value['st_stageid']);
                                                ...
                                                ?>
                                                    ...
                                                    if ($get_planreceive != "") {
                                                        ...
                                                        $sdate = formatDate($p_date, 'd-M-y');
                                                        $ltime = formatDate($act_target, 'd-M-y');
                                                        $start = new DateTime($sdate);
                                                        $end = new DateTime($ltime);
                                                        $bdate = round(($end->format('U') - $start->format('U')) / (60 * 60 * 24)) . ' days';
