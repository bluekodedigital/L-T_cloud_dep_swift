...
$package_details = $cls_user->package_details($pack_id, $projid);
...
$get_po_wo_app = $cls_user->get_po_wo_app($pack_id);
...
$pcrdate = isset($package_details['pm_createdate']) && $package_details['pm_createdate'] instanceof DateTime ? date('d-M-Y', strtotime($package_details['pm_createdate']->format('Y-m-d'))) : '';
    ...
    <div class="row" id="daterow" style=" font-size: 16px;">
        <div class=" col-md-3" id="pdc">
            <small>Current Status:- <span
                    class="badge badge-pill badge-success font-12 text-white ml-1"><?php echo $package_details['shot_name']; ?></span></small>
        </div>
        <?php if ($vendor_details['sup_name'] != "") { ?>
            <div class=" col-md-3" id="pdv" style=" margin-top: -0.8% !important; margin-left: 50% !important; ">
                <small>Vendor:- <span
                        class="badge badge-pill badge-primary font-12 text-white ml-1"><?php echo $vendor_details['sup_name']; ?></span></small>
            </div>
        <?php }
        ?>
...
<div class="modal-body">
    ...
    <div id="wizard" class="form_wizard wizard_horizontal">
        <ul class="wizard_steps">
            <?php
            $repo_firstrow = $cls_user->repo_firstrow($pack_id, $projid, $flg);
            ...
            foreach ($first as $key => $value) {
                ...
                <li>
                    <a href="#step-<?php echo $key + 1; ?>">
                        ...
                        ?>
                        <span class="step_no  <?php echo $completed; ?> <?php echo $bgcolclass ?>"
                            id="stage_<?php echo $value['active'] ?>"><?php echo $key + 1; ?></span>
                        ...
                        <div class="container">
                            <p class="palnned">
                                <span
                                    class="badge badge-pill badge-primary font-medium text-white ml-1"><?php echo formatDate($value['revised_planned_date'], 'd-M-y'); ?>
                                    </span>
                            </p>
                            <?php $get_actualreceive = $cls_user->get_actualreceive($pack_id, $value['ps_stageid']);
                            ...
                            <p class="actual">
                                ...
                                <?php //if (!empty($value['ps_actualdate']) && strtotime($value['ps_actualdate']) !== false) { ?>
                                <?php if (!empty($value['ps_actualdate'])) { ?>
                            ...
                            <p class="deviations">
                                ...
                                if ($stageid >= $value['ps_stageid']) {
                                    if ($value['days'] < 0) {
                                        $devi = $value['days'];
                                        ?>
                                        <span class="badge badge-pill badge-success font-medium text-white ml-1">
                                            <?php echo $value['days'];
                                            ?></span>
                                        <?php
                                        ...
                                        ?>
                                            <span
                                                class="badge badge-pill badge-danger font-medium text-white ml-1"><?php echo '+ ' . $value['days']; ?></span>
                                        <?php
                                    } else if ($value['days'] = 0) {
                                        $devi = $value['days'];
                                        ?>
                                                <span
                                                    class="badge badge-pill badge-inverse font-medium text-white ml-1"><?php echo '+ ' . $value['days']; ?></span>
                                        <?php
                                    } else {
                                        $devi = $deviations;
                                    }
                            ...
                            <?php } else { ?>
                                <p class="deviations1">
                                    ...
                                    $ltime = date('Y-m-d', strtotime($value['ps_actualdate']));
                                    $start = new DateTime($sdate);
                                    $end = new DateTime($ltime);
    ...
    <div id="wizard" class="form_wizard wizard_horizontal">
        <ul class="wizard_steps">
            ...
            <?php
            $repo_secondrow = $cls_user->repo_secondrow($pack_id, $projid, $poc);
            // if ($po_wo == 0) {
            ...
            foreach ($second as $key => $value) {
                ...
                <li>
                    <a href="#step-<?php echo $key + 10 + 1; ?>">
                        ...
                        ?>
                        <span class="step_no <?php echo $completed; ?> <?php echo $bgcolclass ?>"
                            id="stage_<?php echo $value['active'] ?>"><?php echo $key + 10 + 1; ?></span>
                        <div class="container" style=" font-size: 12px;">
                            <span class="step_descr">
                                <?php echo $status; ?>
                            </span>
                        ...
                        <div class="container">
                            <p class="palnned">
                                <span
                                    class="badge badge-pill badge-primary font-medium text-white ml-1"><?php echo isset($value['revised_planned_date']) && $value['revised_planned_date'] instanceof DateTime ? date('d-M-y', strtotime($value['revised_planned_date']->format('Y-m-d'))) : ''; ?></span>
                            </p>
                            <?php $get_actualreceive = $cls_user->get_actualreceive($pack_id, $value['ps_stageid']);
                            if ($get_actualreceive == '0') {
                            } else { ?>
                            ...
                            <?php if (isset($value['ps_actualdate']) && $value['ps_actualdate'] instanceof DateTime) { ?>
                                    <span class="badge badge-pill badge-info font-medium text-white ml-1">
                                        <?php echo isset($value['ps_actualdate']) && $value['ps_actualdate'] instanceof DateTime ? date('d-M-y', strtotime($value['ps_actualdate']->format('Y-m-d'))) : ''; ?>
                                    </span>
                                <?php } ?>
                            ...
                            <?php } else { ?>
                                <p class="deviations1">
                                    <?php
                                    $sdate = isset($get_actualreceive) && $get_actualreceive instanceof DateTime ? date('d-M-y', strtotime($get_actualreceive->format('Y-m-d'))) : '';
                                    $ltime = isset($value['ps_actualdate']) && $value['ps_actualdate'] instanceof DateTime ? date('d-M-y', strtotime($value['ps_actualdate']->format('Y-m-d'))) : '';
                                    $start = new DateTime($sdate);
                                    $end = new DateTime($ltime);
                                    $bdate = round(($end->format('U') - $start->format('U')) / (60 * 60 * 24)) . '';
