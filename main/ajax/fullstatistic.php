...
$archart = array(array("Date", "Visits"));
$archart1 = array(array("Project", "Visits"));
?>
<div style=" border: 1px solid #003f72;border-top: 0;float: left;width: 100%;">
    <button class=" btn btn-primary" id="fullstatsexp" style="position: absolute; margin: 2px;">Export</button>
    ...
    <div class="col-md-7 table-responsive" id="dvData" style="height: 400px; overflow-y: auto; overflow-x: scroll;">
        <table class="table" style=" white-space:  nowrap;">
            ...
            <tbody>
                <?php
                $fdate = strtotime($_POST['from']);
                $from = date('Y-m-d', $fdate);
                $tdate = strtotime($_POST['to'] . " +1 day");
                $to = date('Y-m-d', $tdate);
                $logq = mssql_query("SELECT *,CONVERT(VARCHAR(8), L_date, 114) as time FROM Log_log WHERE L_date>'$from' AND L_date<'$to' AND L_module = '$mod' ORDER BY L_date DESC");
