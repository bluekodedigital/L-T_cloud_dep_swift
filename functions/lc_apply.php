...
if (isset($_POST['lc_apply'])) {
    ...
    if ($today > $end_lc) {
        ...
        $sql = "insert into swift_lc_entry(lce_id,lcmst_id,lce_packid,lce_venid,lce_projid,lce_ponum,lce_applied,lce_ttl,lce_balance,lce_applydate,lce_remarks,po_wo_flag)
            values('" . $id . "','" . $lc_mst_id . "','" . $lc_pack_id . "','" . $lc_venid . "','" . $lc_proj_id . "','" . $lpo_number . "','" . $lc_value . "','" . $lc_ttl . "','" . ($lc_ttl - $lc_value) . "',GETDATE(),'" . $remarks . "','1')";
        ...
        $sql = "UPDATE swift_packagestatus SET ps_expdate = '$expected',ps_actualdate=GETDATE(),ps_userid='$senduserid',ps_remarks='$remarks',updated_date=GETDATE(),active='0' WHERE ps_packid = '$lc_pack_id' AND ps_stageid = '$recvstageid'";
        $query = mssql_query($sql);
