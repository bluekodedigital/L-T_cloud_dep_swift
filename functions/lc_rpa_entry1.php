<?php 
    ...
    if(isset($_POST['lc_create'])){
        ...
        $query = mssql_query($sql);
        if($query > 0){
            $sql="UPDATE swift_lc_rpa SET lc_number='".$lc_number."',lc_value='".$lc_value."',lc_valid_from='".$valid_from."',lc_valid_to='".$valid_to."' WHERE lr_packid='$pack_id'";
            $query = mssql_query($sql);
        }else{
         ...
         $query = mssql_query($sql);
         if($query){
            // $sql = "UPDATE swift_checklist_mapping SET cm_lc='2' WHERE cm_id='$row_id'";
            // $query = mssql_query($sql);
        ...
        $query = mssql_query($sql);
        if($query > 0){
            $sql = "UPDATE swift_lc_rpa SET rpa_number='".$rpa_number."',rpa_value='".$rpa_value."',rpa_valid_from='".$valid_from."',rpa_valid_to='".$valid_to."' WHERE lr_packid='$pack_id'";
            $query = mssql_query($sql);
        }else{
            // INSERT DATAS INTO SWIFT_LC_RPA
            $sql = "SELECT ISNULL(MAX(lr_id), 0)+1 AS id FROM swift_lc_rpa";
         ...
         $query = mssql_query($sql);
         if($query){
            // $sql = "UPDATE swift_checklist_mapping SET cm_rpa='2' WHERE cm_id='$row_id'";
            // $query = mssql_query($sql);
             echo "<script>window.location.href='../lc_page';</script>";
         }else{
             echo "not insert";
