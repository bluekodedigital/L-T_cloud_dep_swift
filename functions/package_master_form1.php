...
//                    header('Location: ../package_master');
                    ...
                    if ($tech_skip == 1) {
                        ...
                        $rsql = mssql_query("select uid from usermst where usertype=14 $seg");
                        $r = mssql_fetch_array($rsql);
                        $rec_uid = $r['uid'];
...
if (isset($_REQUEST['package_update'])) {
    ...
    if ($num_rows > 0) {
        ...
        if ($update) {
            ...
            $countt = sizeof($mat_required_1);
            for ($i = 1; $i <= $countt; $i++) {
                $sql = "update swift_packagestatus set ps_planneddate='" . $mat_required_1[$i] . "',revised_planned_date='" . $mat_required_1[$i] . "' where ps_packid='" . $pck_id . "' and ps_projid='" . $proj_name . "' and ps_stageid='" . $i . "'  ";
                mssql_query($sql);
            }
