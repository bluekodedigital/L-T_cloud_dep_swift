                ...
                <div class="material-card card">
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="file_export" style=" white-space: nowrap !important;" class="table table-bordered">
                                <thead>
                                    <tr>            
                                ...
                                <tbody>
                                    <?php
                                    $result = $cls_comm->select_techexpert_repository($pid, $_SESSION['uid'],$seg);
                                    $res = json_decode($result, true);
                                    foreach ($res as $key => $value) {
                                        ...
                                        $getid = $value['txp_id'];
                                              ...
                                              $job_code = $super_admin->job_code($value['txp_projid']);
                                        ?>
                                        <tr>
                                            ...
                                            <td class="">
                                                ...
                                                if ($num_rows > 0) {
                                                    ?>
                                                    <span class="pointer" onclick="view_attachments('<?php echo $projname; ?>', '<?php echo $packname; ?>', '<?php echo $value['txp_projid']; ?>', '<?php echo $value['txp_packid']; ?>', '<?php echo $job_code; ?>')" data-toggle="modal" data-target="#exampleModal" data-whatever="@mdo"> <i class="fas fa-paperclip text-black"></i> </span>
                                                <?php }else{
                        ...
                        <div class="modal-body">
                            <form>  
                                        ...
                                        <input type="hidden" value="" name="packageid" id="packageid">
                                         <div class="form-row">
                                        <input type="hidden" value="" id="jbcode">
                                        <div class="col-md-2 mb-2 none">
                                            <select class="custom-select" name="doc_type" id="doc_type" >
                                                ...
                                                foreach ($res as $key => $value) {
                                                    ?>
                                                    <option value="<?php echo $value['dt_doctype']; ?>"><?php echo $value['dt_docname']; ?> </option> 
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <div class="col-md-2 mb-2 none">
                                            <select class="custom-select" name="team_code" id="team_code" >
                                                <option value=""> Team Code</option> 
                                                <?php
                                                $result = $super_admin->select_teamcode();
                                                $res = json_decode($result, true);
                                                foreach ($res as $key => $value) {
                                                    ?>
                                                    <option value="<?php echo $value['tc_teamcode']; ?>"><?php echo $value['tc_teamname']; ?> </option> 
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <div class="col-md-3 mb-3 none">
                                            <select class="custom-select" name="setcode" id="setcode" >
                                                <option value="">Doc.Set Code</option> 
                                                <?php
                                                $result = $super_admin->select_docset();
                                                $res = json_decode($result, true);
                                                foreach ($res as $key => $value) {
                                                    ?>
                                                    <option value="<?php echo $value['dc_setcode']; ?>"><?php echo $value['dc_setname']; ?> </option> 
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <div class="col-md-3 mb-3 none">
                                            <select class="custom-select" name="syscode" id="syscode" >
                                                <option value="">System Code</option> 
                                                <?php
                                                $result = $super_admin->select_systemcode();
                                                $res = json_decode($result, true);
                                                foreach ($res as $key => $value) {
                                                    ?>
                                                    <option value="<?php echo $value['sc_code']; ?>"><?php echo $value['sc_name']; ?> </option> 
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <div class="col-md-2 mb-2 none">
                                              <div class="input-group-append">
                                                <button class="btn btn-success " type="button" id="gen_doc_num" onclick="gen_doc_number()">GO</button>
                                            </div>
                                        </div>
                                    </div>
...
<!--                                            <select class="custom-select" name="doc_name" id="doc_name"  required="">
                                            ...
                                            <input type="text" readonly="" class="form-control" id="doc_name" name="doc_name"    placeholder="Enter Document Name">
...
?>
<script>
    function view_attachments(a, b, c, d,jobcode) {
        $("#proj").text(a);
        $("#pack").html(b);
        $("#projectid").val(c);
        $("#packageid").val(d);
